...
class AttModel(CaptionModel):
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        for i in range(seq.size(1) - 1):
            ...
            if self.att_type == "intra" and i != 0:
                output, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, states)
            else:
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, *args):
        ...
        if args == ():
            ...
            states = args[0]
            #inputxs = [self.embed(elem) for elem in args[1]]
            output, state = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks, states)
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        for t in range(self.seq_length + 1):
            ...
            if self.att_type == "intra":
                logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state, states)
            else:
...
class IntraAttention(nn.Module):
    ...
    def forward(self, h, c, att_feats, p_att_feats, att_masks=None, *args):
        if args != ():
            # intra attention,
            states = args[0]
            # inputxs = args[1] # each element is (bs, input_encoding_size)
            ...
            # all_xs = [elem.unsqueeze(1) for elem in inputxs]  # batch, 1, input_encoding_size
            # prev_xs = torch.cat(all_xs, 1)             #bs, t, input_encoding_size
            ...
            et = et + dec_fea                           # bs, t, att_hid_size
            et = F.tanh(et)                             # bs, t, att_hid_size
            et = et.view(bs*t, -1)                      # bs*t, att_hid_size
            et = self.ha(et).squeeze(1).view(bs, t)      # bs, t
            at = F.softmax(et, dim = 1).unsqueeze(1)    # bs, 1, t
            ht_d = torch.bmm(at, prev_h).squeeze(1)     # bs, rnn_size

            '''
            all_pre_c = [states[i][1][-1] for i in range(len(states))]  # each element is (bs, rnn_size)
            all_pre_c = [elem.unsqueeze(1) for elem in all_pre_c]  # batch , 1,  rnn_size
            prev_c = torch.cat(all_pre_c, 1)            # batch, 1, rnn_size
            prev_cc = prev_c.view(-1, prev_c.size()[-1])  # bs*t, rnn_size
            et = self.W_prevc(prev_cc)                   # bs*t, att_hid_size
            et = et.view(bs, t, self.att_hid_size)      # bs, t, att_hid_size
            dec_fea = self.W_c(c).unsqueeze(1).expand_as(et)           # bs, 1, att_hid_size -> bs ,t, att_hid_size            h size is: bs, rnn_size
            et = et + dec_fea                           # bs, t, att_hid_size
            et = F.tanh(et)                             # bs, t, att_hid_size
            et = et.view(bs*t, -1)                      # bs*t, att_hid_size
            et = self.ca(et).squeeze(1).view(bs, t)      # bs, t
            at = F.softmax(et, dim = 1).unsqueeze(1)    # bs, 1, t
            ct_d = torch.bmm(at, prev_c).squeeze(1)     # bs, rnn_size
            '''

            # xs_res = torch.bmm(at, prev_xs).squeeze(1)           # bs, input_encoding_size
...
class IntraAtt2in2Core(nn.Module):
    def __init__(self, opt):
        ...
        self.x2a = nn.Linear(self.input_encoding_size, 2 * self.rnn_size)
